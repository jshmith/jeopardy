...
function targetPosition(sync: VideoSyncState): number {
  return sync.status === 'playing' ? sync.time + (Date.now() - sync.setAtMs) / 1000 : sync.time
}
...
function HostFileVideo({ url, roomCode }: { url: string; roomCode: string }) {
  ...
  function publish(status: VideoSyncState['status']) {
    const v = ref.current
    if (!v) return
    void setVideoSync(roomCode, { status, time: v.currentTime, setAtMs: Date.now() })
  }
...
function ViewerFileVideo({ url, sync, muted }: { url: string; sync: VideoSyncState | null; muted: boolean }) {
  const ref = useRef<HTMLVideoElement>(null)
  ...
  useEffect(() => {
    ...
    if (!v || !sync) return
    const apply = () => {
      const target = targetPosition(sync)
...
function ViewerYouTube({ videoId, sync, muted }: { videoId: string; sync: VideoSyncState | null; muted: boolean }) {
  const containerRef = useRef<HTMLDivElement>(null)
  const playerRef = useRef<YTPlayer | null>(null)
